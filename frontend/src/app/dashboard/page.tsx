...
function DashboardContent() {
  ...
  // Refs — always hold latest values, avoids stale closures in async callbacks
  ...
  // ── Debounced recalc (reads from refs — no stale closure) ─────────────────
  ...
  // ── Load full analysis for a product ──────────────────────────────────────
  // Defined BEFORE initializeDashboard so it can be called inside it
  const loadProductAnalysis = useCallback(
    async (
      ...
      // Product-level stats (fire and forget)
      api.data
      ...
      try {
        ...
        if (!sessionIdFromUrl) {
          try {
            ...
            setSessionCreatedAt(defaultSession.created_at);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-start justify-between flex-wrap gap-3">
            ...
            <div className="text-right text-xs text-slate-400">
              ...
              {sessionCreatedAt && (
                <div className="mt-0.5">Creada: {sessionCreatedAt}</div>
              )}
              {sessionMeta?.date_range && (
                <div className="mt-0.5">
                  Datos: {sessionMeta.date_range.start.slice(0, 10)} —{" "}
                  {sessionMeta.date_range.end.slice(0, 10)}
                </div>
              )}
            </div>
          </div>
